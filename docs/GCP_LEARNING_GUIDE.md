...
Open [BigQuery Studio in the dedicated project](https://console.cloud.google.com/bigquery?project=workforce-replay-yv-260913). Select **SQL query** (or an empty **Untitled query** tab), then paste the [executed evaluation SQL](../cloud/evidence/bigquery-evaluation.sql). In the editor, choose **Edit → Query settings → Advanced options** and set **Maximum bytes billed** to **1073741824 (1 GiB)**. Under Resource management, turn off **Use cached results** for an actual repeat computation. Save the settings; the editor should estimate approximately **3.58 MB**.

Click the blue **▶ Run** button above the SQL. Once results appear, open **Job information** in the results pane to find the job ID. It is a receipt generated by Google, not a value you need to invent. Compare the selected `previous_7` row:
...
During this deployment, the query and bounded settings were prepared in the console, but the owner could not locate the browser page. User execution is therefore not recorded as completed. The links and steps above allow the exercise to be repeated independently.
